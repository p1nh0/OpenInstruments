...
var max_path; // MaxMSP directory (Dict seems to be needing a file path in order to load a dictionary) 
// so we have to store Max's path 
...
{ 
	...
	importobjref(max_path+"Cycling '74/interfaces/obj-qlookup.json"); // import object lookup dictionary 
	makeobjref();
 	o.export_json(max_path+"Cycling '74/interfaces/obj-qlookup.json"); // save file
	
	importdocref(max_path+"Cycling '74/interfaces/doc-qlookup.json"); // import doc lookup dictionary
	makedocref();
	d.export_json(max_path+"Cycling '74/interfaces/doc-qlookup.json"); // save file
...
function savechanges()						
{
	...
	sobj.export_json("oi-obj-lookup.json"); 
	sdoc.export_json("oi-doc-lookup.json"); 
}

function setmaxpath(m)
{
	max_path = new String();
 	max_path = m; 
	post("max path: "+max_path+"\n"); 
} 
function settype(t) 
{
...
function save()
{
	embedmessage("setmaxpath", max_path); 
